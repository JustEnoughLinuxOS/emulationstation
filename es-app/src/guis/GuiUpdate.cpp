	...
	switch (mState)
	{
		case 1:
		{
			...
			if (!mUpdateVersion.empty())
				message = Utils::String::format(_("THE SYSTEM WILL DOWNLOAD AND INSTALL THE UPDATE, CONTINUE?").c_str(), ApiSystem::getInstance()->getVersion().c_str(), mUpdateVersion.c_str()),
